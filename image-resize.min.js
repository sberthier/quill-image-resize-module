...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__modules_DisplaySize__ = __webpack_require__(38);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__modules_Toolbar__ = __webpack_require__(40);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__modules_Resize__ = __webpack_require__(39);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6__Matrix__ = __webpack_require__(37);
var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }();
...
var ImageResize = function ImageResize(quill) {
  ...
  */
		...
		var tS = rotation >= 0 ? t : -t;

		// https://stackoverflow.com/questions/9971230/calculate-rotated-rectangle-size-from-known-bounding-box-coordinates
		var containerWidth = 1 / (Math.pow(Math.cos(tS), 2) - Math.pow(Math.sin(tS), 2)) * (containerRect.width * Math.cos(tS) - containerRect.height * Math.sin(tS));
		var containerHeight = 1 / (Math.pow(Math.cos(tS), 2) - Math.pow(Math.sin(tS), 2)) * (-containerRect.width * Math.sin(tS) + containerRect.height * Math.cos(tS));
		var imgWidth = 1 / (Math.pow(Math.cos(tS), 2) - Math.pow(Math.sin(tS), 2)) * (imgRect.width * Math.cos(tS) - imgRect.height * Math.sin(tS));
		var imgHeight = 1 / (Math.pow(Math.cos(tS), 2) - Math.pow(Math.sin(tS), 2)) * (-imgRect.width * Math.sin(tS) + imgRect.height * Math.cos(tS));
		...
		var cx = containerHeight * Math.sin(tS);
		var cy = containerWidth * Math.sin(tS);
		var ix = imgHeight * Math.sin(tS);
		var iy = imgWidth * Math.sin(tS);
		...
		var p1X = rotation >= 0 ? cx : 0;
		var p1Y = rotation < 0 ? cy : 0;
		var p2X = imgRect.left - containerRect.left + (rotation >= 0 ? ix : 0);
		var p2Y = imgRect.top - containerRect.top + (rotation < 0 ? iy : 0);
